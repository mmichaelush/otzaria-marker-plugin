...
הרישום מתבצע פעם אחת ב־`plugin.boot` והפריט נשאר רשום לכל אורך הסשן — ה־host מציג אותו רק בהקשרי בחירה, כך שאין צורך להסירו כשאין בחירה. אירועי בחירה רק מעדכנים את התוכן (`reader.updateContextMenuItem`) — למשל הוספת "הסר סימון" כשיש הדגשה חופפת. רישום מלא מחדש קורה רק כשסוג התפריט משתנה או כשעדכון נכשל. כל מוטציה של התפריט עוברת דרך `queueMenuTask`, תור סדרתי שמונע משתי זרימות מקבילות להסיר ולהוסיף את אותו פריט זו על גבי זו.
